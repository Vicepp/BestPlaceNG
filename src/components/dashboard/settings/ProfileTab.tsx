...
/** Profile details + dashboard view switcher + account info. */
export default function ProfileTab() {
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={handleSave} className="space-y-4 rounded-2xl border border-zinc-100 bg-white p-6 shadow-sm">
        <div>
          <h2 className="text-lg font-bold text-foreground">Profile</h2>
          <p className="text-sm text-zinc-400">How you appear to landlords, tenants and on your listings.</p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input value={businessName} onChange={(e) => setBusinessName(e.target.value)} placeholder="Shown on your listings"
              className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm outline-none focus:border-brand" />
          </div>
          <div>
        ...
        <div>
          ...
          <input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Your office or home address"
            className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm outline-none focus:border-brand" />
        </div>
      ...
      <div className="rounded-2xl border border-zinc-100 bg-white p-6 shadow-sm">
        ...
        <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
          ...
          <div>
            ...
            <p className="mt-0.5 text-sm text-foreground">{profile?.createdAt ? new Date(profile.createdAt).toLocaleDateString() : "—"}</p>
          </div>
        </div>
      </div>
    </div>
